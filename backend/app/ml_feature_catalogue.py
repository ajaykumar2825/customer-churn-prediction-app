"""The ML feature contract shared by prediction and explanation services.

Single source of truth for accepted values so the API can validate payloads
and generate human-readable labels at the same time.
"""
...
BOOL_FEATURES = [name for name, spec in ML_FEATURE_CATALOGUE.items() if spec["kind"] == "bool"]
ENUM_FEATURES = [name for name, spec in ML_FEATURE_CATALOGUE.items() if spec["kind"] == "enum"]
NUMERIC_FEATURES = [name for name, spec in ML_FEATURE_CATALOGUE.items() if spec["kind"] in {"int", "number"}]

FEATURE_ORDER = list(ML_FEATURE_CATALOGUE.keys())
